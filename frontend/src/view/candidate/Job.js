...
function Job() {
  ...
  // State và hook
  /**id: Lấy ID của job từ URL bằng useParams().
  nav: Dùng để điều hướng đến các trang khác thông qua useNavigate().
  job: Trạng thái chứa thông tin chi tiết của job, bao gồm thông tin của nhà tuyển dụng (employer), loại công việc (jtype), cấp bậc (jlevel), và danh sách ngành nghề (industries).
  user: Lấy thông tin người dùng từ Redux store.
  isAuth: Trạng thái kiểm tra xem người dùng đã đăng nhập hay chưa.
  isApplied, isUpload, isSaved: Các trạng thái kiểm tra xem người dùng đã ứng tuyển công việc, đã tải lên CV hay đã lưu công việc này chưa.
  file: Trạng thái lưu trữ file CV của người dùng khi họ tải lên để ứng tuyển.
  industries: Danh sách ngành nghề liên quan đến công việc hiện tại. */
  ...
  /**getJobInf: Lấy thông tin chi tiết về công việc từ API (jobApi.getById(id)), sau đó cập nhật vào state job và industries. */
  ...
  /**checkApplying: Kiểm tra xem người dùng đã ứng tuyển vào công việc này chưa thông qua API (jobApi.checkApplying(id)), cập nhật trạng thái isApplied */
  ...
  /**handleApply: Xử lý khi người dùng nhấn vào nút "Ứng tuyển". Hàm này sẽ gửi dữ liệu CV đã tải lên đến API và thông báo kết quả ứng tuyển thành công. */

  const getFileInf = (e) => {
    setFile(e.target.files[0]);
  };
  /**getFileInf: Lấy thông tin file khi người dùng chọn CV để tải lên */
  ...
  /**checkLoggedIn: Kiểm tra xem người dùng đã đăng nhập chưa, nếu chưa thì thông báo người dùng đăng nhập. */
  ...
  /**checkJobSaved: Kiểm tra xem công việc này đã được lưu vào danh sách yêu thích của người dùng hay chưa, cập nhật trạng thái isSaved. */
  ...
  /**handleClickSaveBtn: Xử lý sự kiện khi người dùng nhấn nút lưu hoặc hủy lưu công việc. Gửi yêu cầu đến API để cập nhật trạng thái lưu việc làm của người dùng. */

  useEffect(() => {
    getJobInf();
  }, []);
  /**useEffect(() => { getJobInf(); }, []): Gọi hàm getJobInf để lấy thông tin công việc khi component được render lần đầu. */
  ...
  /**Gọi các hàm checkApplying và checkJobSaved để kiểm tra trạng thái của người dùng (đã ứng tuyển, đã lưu công việc) khi người dùng đã đăng nhập. */
